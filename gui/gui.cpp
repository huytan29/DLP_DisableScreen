...
void ReloadWindowsList()
{
	...
	g_TimerID = SetTimer(g_hWnd, 1, 10000, nullptr);
	windowsList.clear();
	...
	EnumWindows([](HWND hWnd, LPARAM) -> BOOL {
		...
		info.hWnd = hWnd;
		HICON hIcon = nullptr;
		auto result = SendMessageTimeoutW(hWnd, WM_GETICON, ICON_SMALL, 1000, SMTO_ABORTIFHUNG, 1000, reinterpret_cast<PDWORD_PTR>(&hIcon));
		if (!result || !hIcon || hIcon == hDefIcon)
		{
			hIcon = nullptr;
			result = SendMessageTimeoutW(hWnd, WM_GETICON, ICON_SMALL2, 1000, SMTO_ABORTIFHUNG, 1000, reinterpret_cast<PDWORD_PTR>(&hIcon));
			if (!result || !hIcon || hIcon == hDefIcon)
			{
				hIcon = reinterpret_cast<HICON>(GetClassLongPtrW(hWnd, GCLP_HICON));
				if (!hIcon || hIcon == hDefIcon)
				{
					result = SendMessageTimeoutW(hWnd, WM_GETICON, ICON_BIG, 1000, SMTO_ABORTIFHUNG, 1000, reinterpret_cast<PDWORD_PTR>(&hIcon));
					if (!result || !hIcon || hIcon == hDefIcon)
					{
						hIcon = reinterpret_cast<HICON>(GetClassLongPtrW(hWnd, GCLP_HICON));
					}
		...
		if (textLen != 0)
		{
			...
			if (GetWindowTextW(hWnd, const_cast<LPWSTR>(info.caption.data()), textLen) == 0)
				info.caption.clear();
		}

		info.className.resize(MAX_PATH);
		if (GetClassNameW(hWnd, const_cast<LPWSTR>(info.className.data()), MAX_PATH) == 0)
...
void LVOnDispInfo(LPNMLVDISPINFOW di)
{
	...
	if (di->item.mask & LVIF_STATE && di->item.iSubItem == 0)
	{
		...
		di->item.state = INDEXTOSTATEIMAGEMASK(info.disabledCapture ? 2 : 1);
	}
...
{
	// Check Create Windows Event or Destroy windows Event
	...
	_In_ int       nCmdShow)
{
	...
	wcex.lpszClassName = L"Windows Cap";
	...
	g_hWnd = CreateWindowExW(0, L"Windows Cap", L"Windows Cap", WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, nullptr, nullptr, hInstance, nullptr);

	if (!g_hWnd)
		return FALSE;

	// Event detected windows event
	HWINEVENTHOOK hWinEventHook = SetWinEventHook(
		EVENT_OBJECT_CREATE,
		EVENT_OBJECT_DESTROY,
		nullptr,  // Handle to the DLL containing the callback (nullptr for local hook)
		WinEventProc,
		0,  // ProcessId, 0 để theo dõi tất cả các tiến trình
		0,  // ThreadId, 0 để theo dõi tất cả các luồng
		WINEVENT_OUTOFCONTEXT | WINEVENT_SKIPOWNPROCESS
	);

	// Hủy đăng ký hook trước khi thoát chương trình
